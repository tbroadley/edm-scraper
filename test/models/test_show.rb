...
class TestShow < Minitest::Test
  def test_show_without_new_or_url_is_valid
    ...
    show = Show.new(name: 'Test', venue: 'Test', start_date: date, end_date: date, filter: false)
    assert show.valid?
  end

  def test_show_with_non_boolean_filter_is_valid
    date = DateTime.new(2005, 1, 1)
    show = Show.new(name: 'Test', venue: 'Test', start_date: date, end_date: date, filter: 'not a boolean')
    assert show.valid?
